...
local function chat_list(msg)
    local data = load_data(_config.moderation.data)
        ...
        local file = io.open("./data/groups/lists/listed_groups.txt", "w")
        file:write(message)
        file:flush()
        file:close()
        return message
...
function run(msg, matches)
  if msg.to.type ~= 'chat' or is_sudo(msg) or is_admin(msg) then
	 local data = load_data(_config.moderation.data)
    if matches[1] == 'join' and data[tostring(matches[2])] then
      ...
      if is_hammer(msg.from.id) then
            return 'You are globally banned.'
      end
      if data[tostring(matches[2])]['settings']['lock_member'] == 'yes' and not is_owner2(msg.from.id, matches[2]) then
        return 'Group is private.'
      end
          local chat = "channel#id"..matches[2]
     ...
     if matches[1] == 'chatlist'then
       if is_admin(msg) and msg.to.type == 'chat' then
         chat_list(msg)
         send_document("channel#id"..msg.from.id, "./data/groups/lists/listed_groups.txt", ok_cb, false)
       elseif msg.to.type ~= 'chat' then
         chat_list(msg)
         send_document("user#id"..msg.from.id, "./data/groups/lists/listed_groups.txt", ok_cb, false) 
       end      
       end
     end
  end
